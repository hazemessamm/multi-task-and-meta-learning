...
class MANN(nn.Module):
    ...
    def forward(self, input_images, input_labels):
        ...
        input_images = input_images.view(-1, self.num_classes*self.samples_per_class, 784)
        input_labels = torch.cat([input_labels[:, :-1, :, :], torch.zeros_like(input_labels[:, -1:, :, :])], dim=1)
        input_labels = input_labels.view(-1, self.num_classes*self.samples_per_class, self.num_classes)
        img_labels_cat = torch.cat([input_images, input_labels], -1)
        out, _ = self.layer1(img_labels_cat)
        out, _ = self.layer2(out)
        out = out.view(-1, self.samples_per_class, self.num_classes, self.num_classes)
        return out
    ...
    def loss_function(self, preds, labels):
        ...
        preds = preds[:, -1:, :, :]
        labels = labels[:, -1:, :, :]
        labels = labels.reshape(-1, self.num_classes).argmax(-1)
        loss = F.cross_entropy(preds.reshape(-1, self.num_classes), labels)
        return loss
        #############################
