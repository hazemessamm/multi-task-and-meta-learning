...
def get_images(paths, labels, nb_samples=None, shuffle=True):
    ...
    if nb_samples is not None:
        ...
        sampler = lambda x: x
    images_labels = [(i, os.path.join(path, image)) for i, path in zip(labels, paths) for image in sampler(os.listdir(path))]
    if shuffle:
...
class DataGenerator(IterableDataset):
    ...
    def _sample(self):
        ...
        labels = [sample.split('/')[-1] for sample in classes]

        img_flattened = 784
        imgs = np.zeros((self.num_samples_per_class, self.num_classes, img_flattened))
        one_hot_labels = np.zeros((self.num_samples_per_class, self.num_classes, self.num_classes))
        for i, (cls, label) in enumerate(zip(classes, labels)):
            labels_imgs_paths = get_images([cls], [label], self.num_samples_per_class, shuffle=True)
            for j, (label, img_path) in enumerate(labels_imgs_paths):
                img = self.image_file_to_array(img_path, img_flattened)
                imgs[j, i, :] = img
                one_hot_labels[j, i, i] = 1
        

        return imgs.astype('float32'), one_hot_labels.astype('int32')
        #############################
